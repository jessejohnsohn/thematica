...
module.exports = class Alias extends Now {
  ...
  updatePathBasedroutes(alias, rules) {return __async(function*(){
    ...
    return yield this.upsertPathAlias(alias, rules)
  }.call(this))}
  ...
  maybeSetUpDomain(alias) {return __async(function*(){
    ...
    if (!/\.now\.sh$/.test(alias)) {
      ...
      const {domain, nameservers} = domainInfo ? {domain: _domain} : yield this.getNameservers(alias)
      ...
      if (!usingZeitWorld && domainInfo) {
        if (domainInfo.verified) {
          ...
          const {verified, created} = yield this.setupDomain(domain, {isExternal: true})
          if (!(created && verified)) {
        ...
        if (err.userError) {
          ...
          try {
            if (usingZeitWorld) {
              ...
              try {
                yield this.retry(() => this.verifyOwnership(alias), {retries: 10, maxTimeout: 8000})
              } catch (err2) {
      ...
      if (!usingZeitWorld && !skipDNSVerification) {
        ...
        const {uid, verified, created} = yield this.setupDomain(domain, {isExternal: true})
        if (!(created && verified)) {
          const e = new Error(`> Failed to verify the ownership of ${domain}, please refer to 'now domain --help'.`)
          e.userError = true
          throw e
