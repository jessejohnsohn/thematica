...
function run(token) {return __async(function*(){
  ...
  if (subcommand === 'ls' || subcommand === 'list') {
    ...
    const record = yield domainRecords.create(domain, {name, type, value, mxPriority})
    const elapsed = ms(new Date() - start)
    ...
    const yes = yield readConfirmation(record, 'The following record will be removed permanently\n')
    if (!yes) {
      error('User abort')
      return exit(0)
    }
