...
function run(token) {return __async(function*(){
  ...
  switch (subcommand) {
    ...
    case 'remove': {
      ...
      try {
        const confirmation = (yield readConfirmation(domain, _domain, _domains)).toLowerCase()
    ...
    case 'set': {
      ...
      const {uid, code, created, verified} = yield domain.add(name, argv.force, argv.external)
      const elapsed = ms(new Date() - start)
      if (created) {
