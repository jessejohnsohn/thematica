...
module.exports = class Now extends EventEmitter {
  ...
  create(path, {
    ...
    const deployment = yield this.retry(bail => __async(function*(){
      ...
      const files = yield Promise.all(Array.prototype.concat.apply([], yield Promise.all((Array.from(this._files)).map(([sha, {data, names}]) => __async(function*(){
        const statFn = followSymlinks ? stat : lstat

        return yield names.map(name => __async(function*(){
          let mode
  ...
  upload() {
    ...
    const uploadChunk = () => {
      Promise.all(parts.shift().map(sha => retry((bail, attempt) => __async(function*(){
      ...
      }.call(this)), {retries: 3, randomize: true, onRetry: this._onRetry})))
  ...
  list(app) {return __async(function*(){
    ...
    const {deployments} = yield this.retry(bail => __async(function*(){
      if (this._debug) {
      ...
      return res.json()
    }.call(this)), {retries: 3, minTimeout: 2500, onRetry: this._onRetry})
  ...
  listAliases(deploymentId) {return __async(function*(){
    return this.retry(() => __async(function*(){
      const res = yield this._fetch(deploymentId ? `/now/deployments/${deploymentId}/aliases` : '/now/aliases')
      const body = yield res.json()
      return body.aliases
    }.call(this)))
  }.call(this))}
  ...
  getNameservers(domain) {
    return new Promise((resolve, reject) => {
      ...
      this.retry((bail, attempt) => __async(function*(){
        ...
        const res = yield this._fetch(`/whois-ns${fallback ? '-fallback' : ''}?domain=${encodeURIComponent(domain)}`)
  ...
  createCert(domain, {renew} = {}) {
    return this.retry((bail, attempt) => __async(function*(){
      ...
      return body
    }.call(this)), {retries: 5, minTimeout: 30000, maxTimeout: 90000})
  }
