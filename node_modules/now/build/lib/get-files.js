...
function npm(path, pkg, nowConfig = null, {
  limit = null,
  hasNowJson = false,
  debug = false
  ...
  const search = Array.prototype.concat.apply([], (yield Promise.all(search_.map(file => glob(file, {cwd: path, absolute: true, dot: true})))))
...
function docker(path, nowConfig = null, {
  limit = null,
  hasNowJson = false,
  debug = false
} = {}) {return __async(function*(){
...
function explode(paths, {accepts, debug}) {return __async(function*(){
  ...
  const many = all => __async(function*(){
    return yield Promise.all(all.map(file => __async(function*(){
      return yield list(file)
    }())))
  }())
